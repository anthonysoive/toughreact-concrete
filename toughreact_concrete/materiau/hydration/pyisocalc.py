...
#
# Example usage:
# ./pyisocalc.py -f 'Fe(ClO3)5' -p y -g 0.25 -o ironperchlorate.dat -c -2 -r 250
# Do "./pyisocalc.py --help" to find out more
#
##########################################################################
ver='0.8b (19 Jan. 2013)'
# Version 0.8 -- the input is now completely different. That's right, this version is
#             NOT backwards compatible.
#         0.8a Further improvements to the input format
#         0.8b Some minor tidying up
# Version 0.7 -- Can now handle 0 in formulae (i.e. K0Cl gets treated as Cl
# Version 0.6 -- fixed a bug that made  -plt required for the generation of spectra
# Version 0.5 --  complete rewrite of molecule parser to hammer out a lingering bug
# Version 0.4 --  sorted out issues with parentheses without multipliers
# Version 0.3c -- updated and checked the isotopic ratios
# Version 0.3b -- Sorted out some pretty damaging bugs. Most elements still need checking
#
#
# Dependencies:
# At a minimum you will need python2.7, python-numpy, python-matplotliba
#
...
from numpy import (  # for molw calc  #for cartesian product  # for binning  # misc math functions
	exp,
	linspace,  #for gaussian
	matrix,
	pi,
	sqrt,
	transpose,
...
# try: 
# 	if "-g" in arguments:
# 		sigma=float(arguments[arguments.index('-g')+1])
# 		print 'Will use a broadening factor of',sigma
# 	elif "--gauss" in arguments:
# 		sigma=float(arguments[arguments.index('--gauss')+1])
# 		print 'Will use a broadening factor of',sigma
# 	if sigma==0:
# 		sigma=0.01
# except:
...
# try: 
# 	if "-r" in arguments:
# 		pts=int(arguments[arguments.index('-r')+1])
# 		print 'Will use ',pts,' for the m/z axis'
# 	elif "--resolution" in arguments:
# 		pts=int(arguments[arguments.index('--resolution')+1])
# 		print 'Will use ',pts,' for the m/z axis'
# except:
# 	sigma=0.35	
# 
